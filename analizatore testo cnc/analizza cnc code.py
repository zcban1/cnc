...
def analizza_file_cnc(nome_file):
    ...
    try:
        with open(nome_file, 'r') as file_cnc:
            ...
            for numero_riga, riga in enumerate(righe, start=1):
                # Trova il comando nella riga
                comando = riga.split()[0].strip()
                
                # Gestione dei commenti allineati
                commento = riga[riga.find('(') + 1:riga.find(')')] if '(' in riga and ')' in riga else None
                if commento:
                    commenti.append(f'Riga {numero_riga}: {commento}')

                # Continua solo se c'è un comando valido
                if comando:
                    parametri = riga.strip().split()[1:]
                    ...
                    elif comando in ["G02", "G03"] and len(parametri) >= 3:
                    ...
                    elif comando in ["G00", "G01", "G02", "G03"] and parametri:
    ...
    return posizioni_x, posizioni_y, commenti

def disegna_percorso(posizioni_x, posizioni_y):
    plt.plot(posizioni_x, posizioni_y, marker='o')
    plt.title('Percorso CNC')
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.show()
...
if __name__ == "__main__":
    ...
    posizioni_x, posizioni_y, commenti = analizza_file_cnc(nome_file_cnc)
    # Stampa i commenti nello stesso numero di righe del file
    print("\nCommenti:")
    for commento in commenti:
        print(commento)
    disegna_percorso(posizioni_x, posizioni_y)
